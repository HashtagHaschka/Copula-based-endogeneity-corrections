...
# Yang et. al. (2024) using the least-squares-based correction function 
# approach.
#
...
#
# Yang, F., Y. Qian, and H. Xie (2024). Addressing endogeneity using a 
# two-stage copula generated regressor approach. EXPRESS: Journal of Marketing 
# Research
...
mod2sCOPE2 <- CopReg2sCOPE(formula = logmove ~ price4 + price1 + price2 + price3 + price5 + price6 + price7 + price8 + price9 + price10,
                           data = dat1_Tropicana, cdf = "ecdf")
mod2sCOPE2[[1]]
hist(mod2sCOPE2[[2]])


mod2sCOPE3 <- CopReg2sCOPE(formula = logmove ~ price4 + price1 + price2 + price3 + price5 + price6 + price7 + price8 + price9 + price10 | feat + as.factor(deal),
                           data = dat1_Tropicana, cdf = "ecdf")
mod2sCOPE3[[1]]
hist(mod2sCOPE3[[2]])


mod2sCOPE4 <- CopReg2sCOPE(formula = logmove ~ price4 + price1 + price2 + price3 + price5 + price6 + price7 + price8 + price9 + price10 | feat + as.factor(deal) + as.factor(store),
                           data = dat1_Tropicana, cdf = "ecdf")
mod2sCOPE4[[1]]
hist(mod2sCOPE4[[2]])
